...
if(!function_exists('it_setup_filters')) {
	...
	function it_setup_filters($disabled_filters, $args, $format) {		
...
if(!function_exists('it_custom_menus')) {
	...
	function it_custom_menus() {
		register_nav_menus(array( 'sticky-menu' => __( 'Sticky Menu',IT_TEXTDOMAIN ), 'section-menu' => __( 'Section Menu',IT_TEXTDOMAIN ), 'secondary-menu' => __( 'Secondary Menu',IT_TEXTDOMAIN ), 'utility-menu' => __( 'Utility Menu',IT_TEXTDOMAIN )));
	}
}
...
if(!function_exists('it_page_in_category')) {
	#determine of the current page is within a managed category
	function it_page_in_category($postid) {	
...
if(!function_exists('it_twitter_count')) {	
	function it_twitter_count($username) {	
		#delete_transient( 'it_twitter_count' ); #manually expire the cached value
...
if(!function_exists('it_facebook_count')) {
	function it_facebook_count($url) {
		#delete_transient( 'it_facebook_count' ); #manually expire the cached value
...
if(!function_exists('it_gplus_count')) {
	function it_gplus_count($url) {
		#delete_transient( 'it_gplus_count' ); #manually expire the cached value
		$count = get_transient( 'it_gplus_count' );
		if($count === false || $count == -1) {	
			$data = file_get_contents($url);
			if($data) {
				#if (preg_match('/>([0-9,]+) people</i', $data, $matches)) { #used in 1.3 and earlier
				if (preg_match('/BOfSxb">([0-9,]+)/', $data, $matches)) {
...
if(!function_exists('it_pinterest_count')) {
	function it_pinterest_count($url) {
		#delete_transient( 'it_pinterest_count' ); #manually expire the cached value
...
if(!function_exists('it_search_args')) {
	function it_search_args($args) {
		if(is_search()) {
			global $wpdb;
