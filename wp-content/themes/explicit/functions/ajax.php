...
if(!function_exists('itajax_user_rate')) {	
	...
	function itajax_user_rate() {
		...
		#make sure user hasn't already rated this criteria
		$ip = it_get_ip();
...
if(!function_exists('itajax_menu_terms')) {	
	...
	function itajax_menu_terms() {
		...
		$menu_args = array('object' => $object, 'objectid' => $objectid, 'object_name' => $object_name, 'loop' => $loop, 'thumbnail' => $thumbnail, 'numarticles' => $numarticles, 'type' => $type);
...
if(!function_exists('itajax_sort')) {	
	...
	function itajax_sort() {
		...
		$largefirst=isset($_POST["largefirst"]) ? $_POST["largefirst"] : '';	
		...
		switch ($loop) {
			...
			case 'main loop':
				#setup loop format
				$format = array('loop' => $loop, 'location' => $location, 'view' => $view, 'columns' => $cols, 'sort' => $sorter, 'paged' => $paginated, 'thumbnail' => $thumbnail, 'rating' => $rating, 'meta' => $meta, 'award' => $award, 'icon' => $icon, 'badge' => $badge, 'excerpt' => $excerpt, 'authorship' => $authorship, 'numarticles' => $numarticles, 'layout' => $layout, 'large_first' => $largefirst);
				switch($sorter) {
					case 'recent':
			...
			case 'menu':
				...
				$content_after = '<a class="view-all" href="' . $term_link . '">' . __('VIEW ALL',IT_TEXTDOMAIN) . '<span class="theme-icon-right-fat"></span></a>';
				
			break;
			case 'sections':
				#setup loop format
				$format = array('loop' => 'sections', 'location' => $location, 'thumbnail' => $thumbnail, 'rating' => $rating, 'meta' => $meta, 'award' => $award, 'icon' => $icon, 'badge' => $badge, 'excerpt' => $excerpt, 'authorship' => $authorship, 'nonajax' => true, 'numarticles' => $numarticles, 'columns' => 1, 'view' => 'list', 'sort' => 'recent', 'large_first' => $largefirst, 'size' => 'portholes');
				$args = array('posts_per_page' => $numarticles, 'order' => 'DESC', 'ignore_sticky_posts' => true, 'cat' => $sorter);
				
				#add more link to content
				$link = get_category_link( $sorter );
				$content_after = '<div class="col-md-12"><a class="styled more-link" href="' . esc_url($link) . '">' . __('View All',IT_TEXTDOMAIN) . '<span class="theme-icon-right-fat"></span></a></div>';
