...
class itOptionGenerator {
	...
	function reactions_meta($value) {
		...
		foreach( $value['options'] as $key => $option ) {
			...
			$out .= '<input type="checkbox" name="' . IT_SETTINGS . '[' . $value['id'] . '][]" value="' . $key . '" id="' . $value['id'] . '-' . $i . '"' . $checked . ' />';
			$out .= '<label for="' . $value['id'] . '-' . $i . '">' . $option['icon'] . '&nbsp;' . esc_html( $option['name'] ) . '</label><br />';
		}
	...
	function categories( $value ) {
		...
		foreach( $categories_keys as $id ) {
			...
			$categories_stickylogo = ( !empty( $val['stickylogo'] ) ) ? stripslashes($val['stickylogo'])  : '';
			$categories_stickylogowidth = ( !empty( $val['stickylogowidth'] ) ) ? stripslashes($val['stickylogowidth'])  : '';
			$categories_stickylogoheight = ( !empty( $val['stickylogoheight'] ) ) ? stripslashes($val['stickylogoheight'])  : '';
			$categories_stickylogohd = ( !empty( $val['stickylogohd'] ) ) ? stripslashes($val['stickylogohd'])  : '';
			...
			# categories sticky logo
			$value_local['name'] = __('Sticky Logo',IT_TEXTDOMAIN);
			$value_local['setting'] = 'stickylogo';
			$value_local['value'] = $categories_stickylogo;
			$value_local['id'] = 'edit-categories-stickylogo-' . $id;
			$out .= $this->upload_nested( $value_local );
			
			# sticky logo width
			$value_local['name'] = __('Sticky Logo Width (Optional)',IT_TEXTDOMAIN);
			$value_local['setting'] = 'stickylogowidth';
			$value_local['value'] = $categories_stickylogowidth;
			$value_local['id'] = 'edit-categories-stickylogowidth-' . $id;
			$out .= $this->upload_nested( $value_local );
			
			# sticky logo height
			$value_local['name'] = __('Sticky Logo Height (Optional)',IT_TEXTDOMAIN);
			$value_local['setting'] = 'stickylogoheight';
			$value_local['value'] = $categories_stickylogoheight;
			$value_local['id'] = 'edit-categories-stickylogoheight-' . $id;
			$out .= $this->upload_nested( $value_local );
			
			# categories sticky logo hd
			$value_local['name'] = __('Sticky HD Logo',IT_TEXTDOMAIN);
			$value_local['setting'] = 'stickylogohd';
			$value_local['value'] = $categories_stickylogohd;
			$value_local['id'] = 'edit-categories-stickylogohd-' . $id;
			$out .= $this->upload_nested( $value_local );
	...
	function builder( $value ) {
		...
		foreach( $builder_keys as $id ) {
			...
			$value_local['toggle'] = $toggle;
			$value_local['target'] = 'cat';
			$value_local['toggle_class'] = 'grid blog hero headliner topten trending explicit portholes';
			...
			$value_local['toggle'] = $toggle;
			$value_local['toggle_class'] = 'grid blog hero headliner topten trending explicit connect menu';
			...
			*/
			...
			$value_local['desc'] = __('For Grid, this setting determines how many total rows of posts are displayed (not necessarily how many total posts are displayed). For Blog and other builders, this setting determines how many total posts are displayed.',IT_TEXTDOMAIN);
			$value_local['setting'] = 'rows';
			$value_local['value'] = $rows;
			$value_local['id'] = 'edit-' . $value['id'] . '-rows-' . $id;
			$value_local['toggle'] = $toggle;
			$value_local['target'] = 'range_number';
			$value_local['toggle_class'] = 'grid blog hero trending explicit portholes';
			$out .= $this->select_nested( $value_local );
			...
			$value_local['toggle_class'] = 'hero headliner topten trending explicit portholes';		
			$out .= $this->checkbox_nested( $value_local );
			$value_local['options'] = array();
	...
	function select_target_options( $type, $nodisable = false ) {
		...
		switch( $type ) {
			...
			case 'featured_transition':
				$transitions = array( 'basic' => __('Basic',IT_TEXTDOMAIN), 'fade' => __('Fade',IT_TEXTDOMAIN), 'mask' => __('Mask',IT_TEXTDOMAIN), 'wave' => __('Wave',IT_TEXTDOMAIN), 'flow' => __('Flow',IT_TEXTDOMAIN), 'scale' => __('Scale',IT_TEXTDOMAIN) );
			...
			case 'author_order':
				...
				break;
			case 'builder':
				$effects = array( 'page-content' => __('Page/Post Content',IT_TEXTDOMAIN), 'portholes' => __('Portholes',IT_TEXTDOMAIN), 'hero' => __('Hero Slider',IT_TEXTDOMAIN), 'headliner' => __('Headliner',IT_TEXTDOMAIN), 'topten' => __('Top Ten',IT_TEXTDOMAIN), 'trending' => __('Trending',IT_TEXTDOMAIN),  'grid' => __('Grid',IT_TEXTDOMAIN), 'blog' => __('Blog',IT_TEXTDOMAIN), 'explicit' => __('Explicit Content',IT_TEXTDOMAIN), 'widgets' => __('Widgets',IT_TEXTDOMAIN), 'connect' => __('Connect',IT_TEXTDOMAIN), 'menu' => __('Utility Menu',IT_TEXTDOMAIN), 'html' => __('Custom Content',IT_TEXTDOMAIN) );
				foreach( $effects as $key => $value ) {
					$options[$key] = $value;
				}
				break;
