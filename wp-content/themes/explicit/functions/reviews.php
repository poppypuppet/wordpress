...
if(!function_exists('it_get_details')) {	
	...
	function it_get_details($postid, $image, $isreview){
		...
		$badgesargs = array('postid' => $postid, 'single' => false, 'badge' => true, 'white' => true, 'wrapper' => false);	
		$awardsargs = array('postid' => $postid, 'single' => false, 'badge' => false, 'white' => true, 'wrapper' => false);		
					...
					if(!empty($name)) { 
						...
						if(!empty($meta)) {
							...
							$d .= '<div class="detail-item">';
								$d .= '<span class="detail-label">'.$name.'</span>';								
		...
		if($flag) {
			...
			$out .= '<div id="overview-anchor" class="details-box-wrapper" style="background-image:url(' . $image . ');">';
				...
				$out .= '<div class="details-box">';			
					...
					#if this is a review, put the title within the post for rich snippet purposes
					if($isreview) {
						$out .= '<div class="detail-item">';
							$out .= '<span class="detail-label">' . __('Item Reviewed',IT_TEXTDOMAIN) . '</span>';
							$out .= '<span class="detail-content entry-title" itemprop="itemReviewed"><p>' . $item_reviewed . '</p></span>';
						$out .= '</div>';
						$out .= '<div class="detail-item">';
							$out .= '<span class="detail-label">' . __('Author',IT_TEXTDOMAIN) . '</span>';
							$out .= '<span class="detail-content vcard author" itemprop="author"><p><span class="fn">' . get_the_author() . '</span></p></span>';
						$out .= '</div>';
						$out .= '<meta itemprop="datePublished" content="' . get_the_date() . '"><span class="updated">' . get_the_date() . '</span>';
					}
					$out .= $d;
					if(!empty($badges)) {
						$out .= '<div class="detail-item">';
					...
					if(!empty($awards)) {
						$out .= '<div class="detail-item awards-wrapper">';
							$out .= '<span class="detail-label">'.__('Awards',IT_TEXTDOMAIN).'</span>';								
			...
			if(!empty($awards) || !empty($badges)) $out.='<div class="awards-badges-wrapper clearfix">';
			if(!empty($awards)) $out .= $awards;
			if(!empty($badges)) $out .= $badges;
			if(!empty($awards) || !empty($badges)) $out.='</div>';		
			#if this is a review, put the title within the post for rich snippet purposes
			if($isreview) {
				$out .= '<div class="snippet-fallback clearfix">';
					$out .= '<div class="detail-item">';
						$out .= '<span class="detail-label">' . __('Item Reviewed',IT_TEXTDOMAIN) . '</span>';
						$out .= '<span class="detail-content" itemprop="itemReviewed"><p>' . get_the_title() . '</p></span>';
					$out .= '</div>';
					$out .= '<div class="detail-item">';
						$out .= '<span class="detail-label">' . __('Author',IT_TEXTDOMAIN) . '</span>';
						$out .= '<span class="detail-content" itemprop="author"><p>' . get_the_author() . '</p></span>';
					$out .= '</div>';
	...
	function it_get_bottom_line($postid) {
		...
		if(!empty($bottomline)) {
			$out .= '<div class="bottomline">';				
				$out .= '<div class="section-subtitle">' . $bottomline_label . '</div>';
				$out .= $bottomline;
			$out .= '</div>';
		}
		return $out;
...
if(!function_exists('it_get_criteria')) {	
	...
	function it_get_criteria($postid, $image){
		...
		$total_label = it_get_setting('review_total_editor_label');
		$user_label = it_get_setting('review_total_user_label');
		...
		$total_label = !empty($total_label) ? $total_label : __('Total Score',IT_TEXTDOMAIN);	
		$user_label = !empty($user_label) ? $user_label : __('User Score',IT_TEXTDOMAIN);
		$cssflag = '';
									...
									if(!$editor_rating_disable) {
										...
										$out.='<div class="col-md-'.$csseditor.' col-sm-'.$csseditor.' col-xs-'.$csseditorxs.' total-rating-wrapper">';
											...
											$out.='<div class="rating-wrapper total" itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating">';
												...
												$out.='<div class="section-subtitle">'.$total_label.'</div>';
											$out.='</div>';
										$out.='</div>';
									...
									if(!$user_rating_disable) {
										...
										$out.='<div class="col-md-'.$cssuser.' col-sm-'.$cssuser.' col-xs-'.$cssuserxs.' total-rating-wrapper">';
											...
											$out.='<div class="rating-wrapper total" data-numratings="'.$numratings.'">';								
												...
												$out.='<div class="section-subtitle">'.$user_label.'</div>';
	...
	function it_get_comment_rating($postid, $commentid) {
		...
		if(!it_get_setting('review_user_rating_disable')) {
			...
			if(!it_get_setting('review_user_comment_rating_disable')) {
				...
				foreach($criteria as $criterion) {
					if(isset($criterion[0])) {
						if(isset($criterion[0]->meta_name)) {
							$meta_name = $criterion[0]->meta_name.'_user';
							$name = $criterion[0]->name;
			...
			if(!empty($pros) || !empty($cons) || !empty($ratings)) {
				$out .= '<div class="comment-rating" style="background:url(' . $overlay_image[0] . ')">';
					$out .= '<div class="comment-rating-overlay clearfix"></div>';
						...
						if(!empty($ratings)) {
							$out.='<div class="comment-procon-wrapper">';				
								...
								foreach($ratings as $key => $value) {
									...
									$out.='<div class="rating-wrapper clearfix">';
